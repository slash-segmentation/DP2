...
def approveHITs():
    ...
    while True:
        print "reading page"
        pageOfHits = mtc.get_reviewable_hits(page_size=100, page_number=pageNumber)
        pageNumber += 1
        if len(pageOfHits) == 0: break
        print "finished reading page"
        for i in pageOfHits:
            ...
            # approve all assignments (required for delete)
            for assignment in mtc.get_assignments(i.HITId):
                print "status of %s:" % i.HITId, assignment.AssignmentStatus
                if assignment.AssignmentStatus == "Submitted":
                    print "approving" 
                    mtc.approve_assignment(assignment.AssignmentId)
...
def createHIT(parameter, maxAssignments, title, layoutIDRealSystem, layoutIDSandbox, requirements):

    if useSandbox:
        layoutID = layoutIDSandbox
    else:
        layoutID = layoutIDRealSystem

    print "using sandbox:", useSandbox
    ...
    for repeat in range(0, 20):
        ...
        #if 1:
            print "creating hit"
            resultSet = mtc.create_hit(hit_layout=layoutID,
                                       ...
                                       lifetime=timedelta(minutes=60*24*2),
                                       approval_delay=timedelta(minutes=60*24*3),
