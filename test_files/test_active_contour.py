...
#inputFile = "O:/images/HPFcere_vol/HPF_rotated_tif/median_then_gaussian_8bit_classified_pixels/out0005.bmp"
#inputFileOriginal = "data/classified_out0005.png"
inputFile = "data/classified_inverted_out0005.png"
#inputFile = "O:/software/InsightToolkit-3.12.0/Wrapping/WrapITK/images/BrainProtonDensitySlice.png"
argv[1:] = [inputFile, "lventricle.png", 81, 114, 5, 1, -0.5, 3, 2]

def main():
    for value in range(0, 48, 4):
        computeActiveContourTest(value)


def computeActiveContourTest(value):
    if len(argv) < 10:
    ...
    sigmoid = itk.SigmoidImageFilter[InternalImageType, InternalImageType].New(gradientMagnitude,
                        OutputMinimum=0.0,
    #sigmoid = itk.SigmoidImageFilter[InternalImageType, InternalImageType].New(reader,
    #                    OutputMinimum=0.0,
			OutputMaximum=1.1,
			Alpha=float(argv[7]),
			Beta=float(argv[8]))
    rescaler = itk.RescaleIntensityImageFilter[InternalImageType, InternalImageType].New(reader,
                   OutputMinimum=0,
               OutputMaximum=1)
    ...
    #geodesicActiveContour = itk.GeodesicActiveContourLevelSetImageFilter[InternalImageType, InternalImageType, InternalPixelType].New(fastMarching, rescaler,
    ...
    itk.write(fastMarching, "GeodesicActiveContourImageFilterOutputFastMarching.nrrd")
    itk.write(sigmoid, "GeodesicActiveContourImageFilterOutputSigmoid.nrrd")
    itk.write(rescaler, "GeodesicActiveContourRescaler.nrrd")
    itk.write(gradientMagnitude, "GeodesicActiveContourImageFilterOutput2.nrrd")
    itk.write(geodesicActiveContour, "GeodesicActiveContour_value%g.nrrd" % value)
    itk.write(gradientMagnitudeRescaler, "GeodesicActiveContourInvertedGradient.nrrd")
